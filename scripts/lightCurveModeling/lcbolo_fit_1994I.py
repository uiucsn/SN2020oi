...
Lbol = pd.read_csv("/Users/alexgagliano/Documents/Research/2020oi/data/reference_data/SN1994I_Lbol.csv")
Lbol
day = Lbol['MJD'] - Lbol['MJD'].values[0]
lum = 10**Lbol['logL']
lumerr = 2.303*lum*(Lbol['mErr'] - Lbol['pErr'])

#some constants
tauNi = 8.77
tauCo = 111.3

tauNis = 8.77*86400 #days to seconds
tauCos = 111.3*86400
...
print()
print(r't$_0$ (MJD) : '+str(np.round(days[0] - t0, 2))+'\n')
print()
print(r'Ejected mass M$_{tot}$ (MSun) : '+str(np.round(Mej/MSun, 2))+r' $\pm$ '+str(np.round(dMej/MSun, 3))+'\n')
print()
print(r'Kinetic energy (FOE) : '+str(np.round(Ekin/1E51, 3))+r' $\pm$ '+str(np.round(dEkin/1E51, 3))+'\n')
print()
...
for i in np.arange(0, 15, 0.1)*86400:
    popt, pcov = curve_fit(L_heat, days+i, lum-lumerr)
    R = np.sum((L_heat(days+i, *popt) - lum)**2)/len(lum)
    R_stat.append(R)
    R_best = np.min(R_stat)
    time_array = np.arange(0, 15, 0.1)*86400
    t0 = time_array[np.where(R_stat == R_best)[0][0]]
    popt, pcov = curve_fit(L_heat, days+t0, lum-lumerr)

popt
pcov

np.nanstd([0.0747809,0.11695242])
0.09586665999999999
0.021085760000000002
...
ax1.set_yscale('linear')
ax1.set_ylabel(r'Luminosity $(erg/s)$', fontsize=15)
ax1.set_xlabel(r'Days from explosion - MJD %.2f' % np.round(days[0] - t0, 2), fontsize=15)
plt.legend()
plt.show()
