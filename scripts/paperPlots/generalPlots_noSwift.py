...
#SN2020oi = pd.read_csv("/Users/alexgagliano/Documents/Research/2020oi/data/photometry/2020oiphotometry_wSynthetic_wForcedZTF_wLCOu_noSwift_final.csv")
...
def make_plot():
    ...
    bands = bands[::-1]
    shifts = np.array([-1.8,  -0.5,  1.0,  2.5,  4.,  5.5,  7.5,  8.,  9.5,  11.])
...
#                            markerfacecolor='w', mec='k',markersize=15, mew=2),
    ...
    ax2.text(54, 13.8-1, r"$z$-2.8", color=cols[0],transform=ax2.transData)
    ax2.text(54, 15.8-1, r"$i$-1.5", color=cols[1],transform=ax2.transData)
    ax2.text(54, 17.3-1, r"$r$", color=cols[2],transform=ax2.transData)
    ax2.text(54.5, 19.1-1, r"$V$+1.5", color=cols[3],transform=ax2.transData)
    ax2.text(54.5, 20.6-1, r"$g$+3.0", color=cols[4],transform=ax2.transData)
    ax2.text(54, 22.7-1, r"$B$+4.5", color=cols[5],transform=ax2.transData)
    ax2.text(55, 24.7-1, r"$u'/u$+6.5", color=cols[6],transform=ax2.transData)
    ...
    ax3.hlines(23, 2.1, 2.9, color='k',  lw=2)
    #with caps
    ax3.vlines(2.1, 22.8,23.2, color='k', lw=2)
    ax3.vlines(2.9, 22.8,23.2, color='k', lw=2)
    ax3.text(2.3, 22.8, r"$\delta t_0$", c='k', fontsize=24)
