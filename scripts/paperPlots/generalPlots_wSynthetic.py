...
#SN2020oi = pd.read_csv("/Users/alexgagliano/Documents/Research/2020oi/data/photometry/2020oiphotometry_wSynthetic_wForcedZTF_final.csv")
#SN2020oi = pd.read_csv("/Users/alexgagliano/Documents/Research/2020oi/data/photometry/2020oiphotometry_wSynthetic_wForcedZTF_wLCOu_final.csv")
SN2020oi = pd.read_csv("/Users/alexgagliano/Documents/Research/2020oi/data/photometry/2020oiphotometry_final.csv")
...
GP_results = pd.read_csv("/Users/alexgagliano/Documents/Research/2020oi/data/interpolated_lcs/GPResults_2020oi_withSBO_final.csv")

specTimes = np.array([58857.3, 58860.548, 58864.6, 58868.6, 58870.5, 58872.4, 58875. , 58876.7,
       58879.4, 58880.3, 58880.4, 58892. , 58895. , 58995.6])
#closest = []
#for spec in specTimes:
#    sub = GP_results[np.abs(GP_results['MJD']- spec)==np.nanmin(np.abs(GP_results['MJD']- spec))]
#    closest.append(sub)
#len(closest)
#len(specTimes)
#closest
#closestPhot = pd.concat(closest, ignore_index=True)
#closestPhot['phase'] = closestPhot['MJD'] - t0
#closestPhot.to_csv("/Users/alexgagliano/Documents/Research/2020oi/data/interpolated_lcs/specPhot.csv",index=False)
...
pred_B = f_B(SN2020oi.loc[(SN2020oi['FLT']=='B') & (SN2020oi['ULIM']==0), 'MJD'].values)
bad_B =  SN2020oi.loc[(SN2020oi['FLT']=='B') & (SN2020oi['ULIM']==0), 'MJD'].values[np.abs(pred_B - SN2020oi.loc[(SN2020oi['FLT']=='B') & (SN2020oi['ULIM']==0), 'MAG'].values)>0.5]
...
pred_V = f_V(SN2020oi.loc[(SN2020oi['FLT']=='V') & (SN2020oi['ULIM']==0), 'MJD'].values)
bad_V =  SN2020oi.loc[(SN2020oi['FLT']=='V') & (SN2020oi['ULIM']==0), 'MJD'].values[np.abs(pred_V - SN2020oi.loc[(SN2020oi['FLT']=='V')& (SN2020oi['ULIM']==0), 'MAG'].values)>0.5]
...
pred_U = f_U(SN2020oi.loc[(SN2020oi['FLT'].isin(['U','u'])) & (SN2020oi['ULIM']==0), 'MJD'].values)
bad_U =  SN2020oi.loc[(SN2020oi['FLT'].isin(['U','u'])) & (SN2020oi['ULIM']==0), 'MJD'].values[np.abs(pred_U - SN2020oi.loc[(SN2020oi['FLT'].isin(['U','u']))& (SN2020oi['ULIM']==0), 'MAG'].values)>0.5]
...
#    GP_results = pd.read_csv("/Users/alexgagliano/Documents/Research/2020oi/data/interpolated_lcs/GPResults_2020oi_withSBO2_Trunc.csv")
    ...
    fig, axs = plt.subplots(2, 3, gridspec_kw={'height_ratios': [1, 3], 'width_ratios':[0.5, 3, 0.5],"hspace":0.3,"wspace":0.2}, figsize=(20,20))
    ax1 = axs[0][1]
    ax2 = axs[1][1]
    ax3 = axs[1][0]
    ax4 = axs[0][0]
    ...
    bands = bands[::-1]
    shifts = np.array([-3.2,  -1.8,  0,  2.5,  4.,  6,  8.5,  -1,  0,  1.])
    #drop shifts by 1 to align r with 0
    ...
    for i in np.arange(len(bands)):
        ...
        for j in np.arange(len(telescopes)):
            ...
            if telescopes[j] == 'Unk':
                ...
                badtemp.loc[badtemp['ULIM'] == 1, 'MAGERR'] = 0.3
            ...
            if len(temp) > 0:
                ...
                if bands[i] in [r'$w2$', r'$m2$', r'$w1$']:
                    ...
                    #only show upper limits for the first 10 days
                #    temp_ulim = temp_ulim[np.abs(temp_ulim['MJD']-t0) < 10]

                    uplims = temp_ulim['ULIM']
                    ax1.errorbar(temp_ulim['MJD']-t0, temp_ulim['MAG']+shifts[i],yerr=temp_ulim['MAGERR'], lolims=uplims, fmt=telescope_dict[telescopes[j]], label=bands[i], mec='k', ms=ms, mew=1, capsize=5.,capthick=2., ecolor=cols[i],mfc=cols[i], alpha=1)
                    temp_reg = temp[temp['ULIM'] == 0]
                    uplims = temp_reg['ULIM']
                    ax1.errorbar(temp_reg['MJD']-t0, temp_reg['MAG']+shifts[i],yerr=temp_reg['MAGERR'], lolims=uplims, fmt=telescope_dict[telescopes[j]], label=bands[i], mec='k', ms=ms, mew=1, capsize=5.,capthick=2., ecolor=cols[i],mfc=cols[i])
                else:
                    uplims = temp['ULIM']
                    temp_reg = temp[temp['ULIM'] == 0]
                    temp_ulim = temp[temp['ULIM'] == 1]
                    #temp_ulim.loc[temp_ulim['ULIM'] == 1, 'MAGERR'] = 0.3
                    temp_ulim = temp_ulim[np.abs(temp_ulim['MJD']-t0) < 10]

                    uplims_reg = temp_reg['ULIM']
                    uplims_ulim = temp_ulim['ULIM']
                    ax2.errorbar(temp_ulim['MJD']-t0, temp_ulim['MAG']+shifts[i],yerr=temp_ulim['MAGERR'], lolims=uplims_ulim, fmt=telescope_dict[telescopes[j]], label=bands[i], mec='k', ms=ms, mew=1, capsize=5.,capthick=2., ecolor=cols[i],mfc=cols[i])
                    #ax3.errorbar(temp_ulim['MJD']-t0, temp_ulim['MAG']+shifts[i],yerr=temp_ulim['MAGERR'], lolims=uplims_ulim, fmt=telescope_dict[telescopes[j]], label=bands[i], mec='k', ms=ms, mew=1, capsize=5.,capthick=2., ecolor=cols[i],mfc=cols[i])
                    ax2.errorbar(temp_reg['MJD']-t0, temp_reg['MAG']+shifts[i],yerr=temp_reg['MAGERR'], lolims=uplims_reg, fmt=telescope_dict[telescopes[j]], label=bands[i], mec='k', ms=ms, mew=1, capsize=5.,capthick=2., ecolor=cols[i],mfc=cols[i])
                    ax3.errorbar(temp_reg['MJD']-t0, temp_reg['MAG']+shifts[i],yerr=temp_reg['MAGERR'], lolims=uplims_reg, fmt=telescope_dict[telescopes[j]], label=bands[i], mec='k', ms=ms, mew=1, capsize=5.,capthick=2., ecolor=cols[i],mfc=cols[i])
            if len(badtemp)>0:
...
#                            markerfacecolor='w', mec='k',markersize=15, mew=2),
    ...
    ax2.text(-2, 21.0, r"$Flux \ Excess$", color='grey', rotation=90, style='italic')

    ax2.text(54, 13.3-1, r"$z$-3.2", color=cols[0],transform=ax2.transData)
    ax2.text(54, 15.5-1, r"$i$-1.8", color=cols[1],transform=ax2.transData)
    ax2.text(54, 17.1-1, r"$r$", color=cols[2],transform=ax2.transData)
    ax2.text(53., 19.8-1, r"$V$+2.5", color=cols[3],transform=ax2.transData)
    ax2.text(54., 21.5-1, r"$g$+4.0", color=cols[4],transform=ax2.transData)
    ax2.text(54, 24.-1, r"$B$+6.0", color=cols[5],transform=ax2.transData)
    ax2.text(52, 27.7-1, r"$u'/u$+8.5", color=cols[6],transform=ax2.transData)

    ax1.text(55, 14.2, r"$w1$-1", color=cols[7],transform=ax1.transData)
    ax1.text(55, 15.2, r"$m2$", color=cols[8],transform=ax1.transData)
    ax1.text(55, 16.4, r"$w2$+1", color=cols[9],transform=ax1.transData)
    ...
    ax3.fill_between(GP_results['MJD']-t0, GP_results['i_LC'] - GP_results['i_std']+shifts[1], GP_results['i_LC'] + GP_results['i_std']+shifts[1],lw=3, alpha=1, color=cols[1])
    ...
    ax2.fill_between(GP_results['MJD']-t0, GP_results['i_LC'] - GP_results['i_std']+shifts[1], GP_results['i_LC'] + GP_results['i_std']+shifts[1],lw=3, alpha=1, color=cols[1])
    ...
    ax6.vlines(383, 11.5, 11.5-0.2173,lw=2, color='k')
    ax6.vlines(383, 11.5, 11.5+0.2415, lw=2, color='k')

    #caps
    ax6.hlines(11.5-0.2173, 381,385, lw=2, color='k')
    ax6.hlines(11.5+0.2415, 381,385, lw=2, color='k')
    ax6.text(380, 12.3, r"$\delta \mu$", c='k', fontsize=24)

    #uncertainty in the extinction estimate
    ax6.vlines(390, 11.5, 11.5+0.17/2., lw=2, color='#7377AB')
    ax6.vlines(390, 11.5, 11.5-0.17/2., lw=2, color='#7377AB')
    #caps
    ax6.hlines(11.5-0.17/2., 388, 392, lw=2, color='#7377AB')
    ax6.hlines(11.5+0.17/2., 388, 392, lw=2, color='#7377AB')
    ax6.text(387., 12.3, r"$\delta E_{BV}$", c='#7377AB', fontsize=24)

    #uncertainty in the time of explosion
    ax3.hlines(26, 2.1, 2.9, color='k',  lw=2)
    #with caps
    ax3.vlines(2.1, 25.8,26.2, color='k', lw=2)
    ax3.vlines(2.9, 25.8,26.2, color='k', lw=2)
    ax3.text(2.3, 25.8, r"$\delta t_0$", c='k', fontsize=24)
    ...
    #plt.savefig("/Users/alexgagliano/Documents/Research/2020oi/img/2020oi_FullPhotometry_forPaperSBO_Trunc_wSynthetic.png",dpi=200, bbox_inches='tight')

make_plot()

#they estimate the absolute V-band peak magnitude of SN 2017ein as −17.47 ± 0.35 mag.
#GP_results = pd.read_csv("/Users/alexgagliano/Documents/Research/2020oi/data/interpolated_lcs/GPResults_2020oi_LateTime.csv")
GP_results = pd.read_csv("/Users/alexgagliano/Documents/Research/2020oi/data/interpolated_lcs/GPResults_2020oi_withSBO_noSwift.csv")
...
Ni = [0.07, 0.11]
Mj = [0.8-0.03, 0.8+0.03]
Ek = [1.38-0.19, 1.38+0.19]
#1.3764 $\pm$ 0.19
Mni_20oi = np.median(Ni)
...
plt.errorbar(Ek_20oi/Mej_20oi, Mni_20oi, fmt='*',yerr=err_Ni,xerr=err_Ek, ms=35, markeredgecolor='k', c=c_20oi, label='SN 2020oi')
plt.plot(Mni.loc[Mni['type'] == 'dark', 'Ek/Mej'], Mni.loc[Mni['type'] == 'dark', 'MNi'], '+', ms=15, mew=3, c=dark_c, markeredgecolor=dark_c, label='faint SNe')
...
# Manually add the first legend back
plt.minorticks_on()
plt.xlabel(r"$E_k/M_{\rm{ej}}$ ($10^{51}$ erg/$M_{\odot}$)")
plt.ylabel(r"$M_{\rm{Ni}56}$ ($M_{\odot}$)")
...
plt.savefig("/Users/alexgagliano/Documents/Research/2020oi/img/Supernova_ExplosionEnergies_ComparisonLines_newCols_08.png",dpi=300,bbox_inches='tight')
